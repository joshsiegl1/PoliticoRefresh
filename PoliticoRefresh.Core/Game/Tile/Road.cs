   ...
   public class Road : Tile
    {
        ...
        public override void Draw(SpriteBatch sbatch, int offsetX, int offsetY)
        {
            ...
            bounds = imageRect;
            sbatch.Draw(ParentTexture, imageRect, null, selectedTint * ChronoCycle.DayColor, 0.0f, Vector2.Zero, SpriteEffects.None, LayerDepth + ChronoCycle.DayAdditive);
            sbatch.Draw(ParentTextureNight, imageRect, null, selectedTint * ChronoCycle.NightColor, 0.0f, Vector2.Zero, SpriteEffects.None, LayerDepth + ChronoCycle.NightAdditive);

            if (type == PieceType.cross || type == PieceType.tri)
            {
                sbatch.Draw(TrafficLight, new Vector2((int)position.X - offsetX, (int)position.Y - offsetY), null, selectedTint * ChronoCycle.DayColor, 0f, Vector2.Zero, 1f, SpriteEffects.None, LayerDepth + 0.2f + ChronoCycle.DayAdditive); //LayerDepth + 0.0011f);
                sbatch.Draw(TrafficLight_Night, new Vector2((int)position.X - offsetX, (int)position.Y - offsetY), null, selectedTint * ChronoCycle.NightColor, 0f, Vector2.Zero, 1f, SpriteEffects.None, LayerDepth + 0.2f + ChronoCycle.NightAdditive);
            }
        }
